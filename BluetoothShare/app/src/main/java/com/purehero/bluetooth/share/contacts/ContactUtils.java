...
public class ContactUtils {
	...
	public static void contactToStringEx( Context context, Cursor contactCursor ) {
		...
		// Get the _ID value
        long mContactId = contactCursor.getLong(contactCursor.getColumnIndex( Contacts._ID ));
        // Get the selected LOOKUP KEY
        String mContactKey = contactCursor.getString( contactCursor.getColumnIndex( Contacts.LOOKUP_KEY ));
	...
	public static byte[] getIconBytes( Context context, long contact_id ) {
		InputStream is = null;
		try {
			is = Contacts.openContactPhotoInputStream(context.getContentResolver(),
                    ContentUris.withAppendedId(Contacts.CONTENT_URI, Long.valueOf(contact_id)));
	...
	public static void openDetailView( Context context, long contact_id ) {
		...
		Uri uri = Uri.withAppendedPath(Contacts.CONTENT_URI, String.valueOf( contact_id ));
		intent.setData(uri);
