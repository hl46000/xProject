...
public class AdbV2 {
	...
	public static BufferedImage screenCaptureEx( AdbDevice device ) {
		...
		String cmd = "shell screencap -p";
		String prog = null;
		if( device == null ) {
			prog = String.format( "%s %s", path, cmd );
		} else {
			prog = String.format( "%s -s %s %s", path, device.getSerialNumber(), cmd );
		}
		...
		try {
			...
			ReplaceFilterInputStream ris = new ReplaceFilterInputStream( input, Collections.singletonMap(new byte[]{ 0x0D, 0x0A}, new byte[]{0x0A}) );
			
			ret = ImageIO.read( ris );
			ByteArrayOutputStream baos = new ByteArrayOutputStream(); 
			
			byte buff [] = new byte[ 102400 ];
			int nRead = 0;
			while(( nRead = input.read( buff, 0, 102400 )) > 0 ) {
	...
	static class ReplaceFilterInputStream extends FilterInputStream {
	     ...
	     * the replac
	     * ements which <code>'from'</code> data has a larger length.
	     */
		/*
