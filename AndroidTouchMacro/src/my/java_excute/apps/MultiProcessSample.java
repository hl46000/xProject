...
public class MultiProcessSample {
	public static void main(String[] args) {
		new MultiProcessSample();
	}
	
	public MultiProcessSample() {
		for( int i = 0; i < 14; i++ ) {
			new Thread( new testProcessThread( String.format( "실행 순서 %d", i))).start();
			//new testProcessThread( String.format( "%d", i)).run();
		}
	}
}

class testProcessThread implements Runnable {
	private String msg;
	public testProcessThread( String _msg ) {
		msg = _msg;
	}
	@Override
	public void run() {
		final JavaProcessBuilder javaProcessBuilder = new JavaProcessBuilder();
		javaProcessBuilder.setWorkingDirectory(".");
		javaProcessBuilder.addClasspathEntry("./bin");
		javaProcessBuilder.setMainClass("my.java_excute.apps.MultiProcessSampleRunner");
		javaProcessBuilder.addArgument( msg );
		...
		try {
			...
			BufferedReader br2 = new BufferedReader(new InputStreamReader(process.getErrorStream(), "UTF-8"));
